...
class ReceiverThread {
 public:
   ...
   * @param socket          Socket that this thread is reponsible for.
   *                        Receiver thread is the owner of this socket
   *                        for the lifetime of the instance
   * @param threadIndex     Every thread is identified by unique index
   * @param protocolVersion The protocol version of the main receiver
   * @param bufferSize      Size of the buffer used by this thread
   ...
   */
  ReceiverThread(ServerSocket &socket, int threadIndex, int protocolVersion,
                 int64_t bufferSize,
  ...
  /// Cleanup of the receiver thread after the transfer is finished
  ErrorCode finish();

  /// Destructor of Receiver thread
  ~ReceiverThread();

  /// Get the port this reeiver thread is listening on
  int32_t getPort() const;

  /// Get the transfer stats recorded by this thread
  const TransferStats &getTransferStats() const;

  /// Does the necessary initialization and starts a std::thread
  void startThread();

  /// Get the perf report collected during the transfer
  PerfStatReport getPerfReport();

  /**
   * Get the metadata for thsi transfer. This metadata is shared among
   * all the receiver threads responsible for the transfer
   */
  Receiver *getMetaData() const;
 ...
 private:
   ...
   * Next states : END
   */
  ReceiverState waitForFinishWithThreadError();
  ...
  /// Report for the performance statistics
  PerfStatReport perfStatReport_;
  ...
  /// Global controller for the threads
  ...
  /// One instance of receiver thread makes a single instance of std::thread
  /// for a transfer
