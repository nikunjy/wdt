...
Sender::Sender(const std::string &destHost, const std::string &srcDir,
  ...
  transferHistoryController_ =
      std::make_shared<TransferHistoryController>(*dirQueue_);
  threadsController_ =
...
int64_t Sender::getProtocolVersion() const {
...
bool Sender::isReadFileChunks() const {
...
std::unique_ptr<TransferReport> Sender::finish() {
  ...
  for (auto &senderThread : senderThreads_) {
    threadStats.push_back(std::move(senderThread->finish()));
  }
  if (!twoPhases) {
    dirThread_.join();
  }
